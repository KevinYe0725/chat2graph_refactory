...
class Workflow(ABC):
    ...
    def __init__(self):
        ...
        self.version_id = uuid4().hex
        #workflow本质就是一个由许多的opeartor构成的流水线
        self._operator_graph: nx.DiGraph = nx.DiGraph()
        self._evaluator: Optional[EvalOperator] = None
        self._operator_task_dict = None
    @property
    def evaluator(self) -> Optional[EvalOperator]:
        """Get the evaluator operator."""
        return self._evaluator
    @property
    def operator_graph(self) -> nx.DiGraph:
        return self._operator_graph
    @property
    def operator_task_dict(self) -> Optional[dict[str, Any]]:
        """Get the operator's task dict."""
        return self._operator_task_dict
