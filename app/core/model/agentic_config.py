...
class AgenticConfig:
    ...
    def _create_from_dict(cls, config_dict: Dict[str, Any]) -> "AgenticConfig":
        ...
        # toolkit configuration (step 1): 创建所有的Tool配置构建，将其注册
        ...
        # toolkit configuration (step 2): 创建action的相关配置及其注册
        ...
        for action_dict in config_dict.get("actions", []):
            ...
            for tool_ref in action_dict.get("tools", []):
                ...
                # 新增字段：model_name, model_type
            model_name = action_dict.get("model_name", "")
            model_type = action_dict.get("model_type", "")
        ...
        # toolkit configuration (step 3): 将Tool和Action联系起来（未来可能需要改🤔）
        ...
        # toolkit configuration (step 4): 构建Toolkit(多个action-Tools链)
