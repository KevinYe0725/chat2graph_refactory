...
from app.core.reasoner.model_service import ModelRegistryService
...
class AgenticService(metaclass=Singleton):
    ...
    def __init__(self, service_name: Optional[str] = None):
        ...
        self._model_registry_service: ModelRegistryService = ModelRegistryService.instance
    ...
    #现在我们需要将其修改成一开始去构建action的同时，要将action的额外的字段 model和model_type 加上
    @staticmethod
    def _build_toolkit(
    ...
    #Leader的工作流保留还算是没有太大的问题🤔,但是后续需要更改
    @staticmethod
        ...
        for action_config_chain in agentic_service_config.toolkit:
            ...
            for action_config in action_config_chain:
                ...
                action = Action(
                    ...
                    name=action_config.name,
                    description=action_config.desc,
                    model_name=action_config.model_name,
    ...
    def _build_expert_workflow(
        ...
        #workflow可用单个operator或者多个operator来构建
    ...
    def configure(self):
        experts: List[Expert] = self._agent_service.list_experts()
        for expert in experts:
            operators: List[Operator] = expert.get_all_operators()
            for operator in operators:
                self._operator_service.register_operator_for_agent(operator.get_operator_config(),expert)

        leader  = self._agent_service.leader
        operators: List[Operator] = leader.get_all_operators()
        for operator in operators:
            self._operator_service.register_operator_for_agent(operator.get_operator_config(),leader)
    ...
    def init_models(self,
        ...
        agentic_service_config = AgenticConfig.from_yaml(yaml_path, encoding)
        self._model_registry_service.init(agentic_service_config.models)
