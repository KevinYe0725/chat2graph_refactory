...
OPERATOR_SUMMARY_PROMPT_TEMPLATE = """
    你是一名分析型推理助手。
请阅读以下上下文内容，提炼其中的关键信息，总结当前任务的进展和核心目标。
...
请基于以上内容，生成第一个 Action 的执行指令，
格式要求：
- 说明 Action 要执行的具体操作（instruction）
- 给出必要输入参数（input_data），用JSON表示。
请以JSON格式返回。
"""
