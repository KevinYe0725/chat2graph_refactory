...
import { DeleteOutlined, EditOutlined, FormOutlined, LayoutFilled, RightOutlined, LeftOutlined } from '@ant-design/icons';
import {
  Attachments,
  Bubble,
  Conversations,
...
import { CURRENT_PREFIXES, FRAMEWORK_CONFIG, LOCAL_STORAGE_SESSION_KEY, LOCAL_STORAGE_STOP_KEY, MESSAGE_TYPE, ROLES } from '@/constants';
...
import logoSrc1 from '@/assets/logo1.png';
...
import GraphDBCreateButton from '@/pages/Graphdb/GraphDBCreateButton';
import KnowledgebaseCreateButton from '@/pages/Knowledgebase/KnowledgebaseCreateButton';
...
const HomePage: React.FC = () => {
  ...
  const { editing, isInit, conversationsItems, activeKey, collapse, content, attachedFiles, headerOpen, uplodaFileIds, closeTag, selectedFramework } = state;
  ...
  const items: GetProp<typeof Bubble.List, 'items'> = parsedMessages?.filter(item => !isEmpty(item?.message)).map((item, idx, all) => {
    ...
    return {
      ...
      avatar: message?.role === 'SYSTEM' ? {
        icon: <img src={logoSrc} />, shape: 'square', size: 40,
        style: {
        border: '2px solid #ffffff', // 这里设置边框
        borderRadius: 8               // 保持跟 square 的圆角一致
        }
      } : undefined,
    ...
    setState((draft) => {
      ...
      draft.conversationsItems = sessions?.map(item => {
        return {
          ...
          group: formatTimestamp(formatMessage, item?.timestamp) || undefined
        } as any
      })
    })
  }, [sessions, activeKey]);
      ...
      <div className={`${styles.sider} ${collapse ? styles['sider-collapsed'] : ''}`}>
        ...
        <div className={styles.title}>
          <span className={styles['title-text']}>
            <img src={logoSrc1} className={styles['title-logo']} />
          </span>
        ...
        <div className={`${styles['go-manager']} ${collapse ? styles['go-manager-collapsed'] : ''}`}>
        <GraphDBCreateButton />
