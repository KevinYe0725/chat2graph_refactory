...
export const useKnowledgebaseEntity = () => {
    ...
    // 加载知识库
    const { run: runSetKnowledgeConfig, loading: loadingSetKnowledgebasesConfig } = useRequest(services.setKnowledgeConfig, {
        manual: true
    })

    // 获取全局知识库
    const { run: runGetGlobalKnowledgebase, loading: loadingGetGlobalKnowledgebase } = useRequest(services.getGlobalKnowledgebase, {
        manual: true,
    });
    ...
    return {
        ...
        runSetKnowledgeConfig,
        loadingSetKnowledgebasesConfig,
        runGetGlobalKnowledgebase,
        loadingGetGlobalKnowledgebase,
    }
}