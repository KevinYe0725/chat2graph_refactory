...
const SenderHeader: React.FC<Props> = (props) => {
  ...
  const { runUploadFile, runGetGlobalKnowledgebase, runSetKnowledgeConfig } = useKnowledgebaseEntity()
  ...
  const beforeUpload = async (file: RcFile) => {
    ...
    const globalKnowledgebase = await runGetGlobalKnowledgebase();
    if (globalKnowledgebase?.success) {
      await runSetKnowledgeConfig({
        knowledgebases_id: globalKnowledgebase?.data?.id,
        file_id: res?.data?.file_id
      }, {
