...
#  pip install --force-reinstall "aiohttp==$target_aiohttp_version" -q 2>&1 | sed 's/ERROR/WARNING/g' >/dev/null
#}
#
#build_python() {
#  app_dir=$1
#
#  cd ${app_dir}
#  poetry lock >/dev/null 2>&1 && poetry install --quiet >/dev/null 2>&1 || fatal "Failed to install python packages"
#  handle_dependency_conflicts
#}
#
#build_web() {
...
#  cd ${web_dir}
#
#  npm cache clean --force >/dev/null 2>&1 && npm install --silent --no-progress --fund=false --audit=false >/dev/null 2>&1 || fatal "Failed to install web packages"
#
#  npm run build --silent --quiet >/dev/null 2>&1 || fatal "Failed to build web packages"
#
