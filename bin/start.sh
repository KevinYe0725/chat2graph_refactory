...
pids=$(get_pids)
if [[ -n $pids ]]; then
  fatal "server already started (pid: $pids)"
fi
...
do
  if [[ "$line" == *"Press CTRL+C to quit"* ]]; then
      pkill -P $$ tail
      echo "Server logs in ${new_server_log_path}"
  else
    echo "$line"
  fi
done
...
pids=$(get_pids)
if [[ -n $pids ]]; then
  info " server started success ! "
else
  fatal "server started failed, logs in ${new_server_log_path}"
fi
